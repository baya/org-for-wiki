...
<%= file_field_tag 'contest[rule_file]', class: 'upload', value: 'a.pdf' %>
file_field_tag 'file', accept: 'text/html', class: 'upload', value: 'index.html'
# => <input accept="text/html" class="upload" id="file" name="file" type="file" value="index.html" />
<%= file_field_tag "contest[rule_file]", value: @preview_contest.rule_file, class: 'file' %>
...
** sanitize
<script>
  console.dir(<%=sanitize @contest.to_json %>);
</script>
** partials
- 参考: http://guides.rubyonrails.org/layouts_and_rendering.html  3.4.1
...
<%= render "partner/shared/contest_form" %>

app/views/partner/shared/_contest_form.html.erb

<%= render partial: "partner/shared/prize_info", locals: {prize: PreviewPrize.new}%>

<%= render partial: "partner/shared/red_report_list", locals: {red_reports: @porn_red_reports, kind_txt: '含有色情内容'} %>
<%= render partial: "partner/shared/red_report_list", locals: {red_reports: @illegal_red_reports, kind_txt: '不符合比赛主题'} %>
<%= render partial: "partner/shared/red_report_list", locals: {red_reports: @false_red_reports, kind_txt: '虚假的陈述'} %>
** 删除一个对象

   @red_report.destroy
** staging app 部署时在本地预先生成资源
...
** 以非 root 用户将应用绑定到 80 端口 ssh 80
- http://ruby-china.org/topics/16094
- http://stackoverflow.com/questions/413807/is-there-a-way-for-non-root-processes-to-bind-to-privileged-ports-1024-on-l
具体方法，如 rails应用正常跑在 3000端口，终端输入：

sudo ssh devuser@localhost -L 80:localhost:3000 -N
sudo ssh jiangguimin@localhost -L 80:localhost:3000 -N
sudo ssh jiangguimin@weibopage.local.fchk.com -L 80:localhost:3000 -N

** 微博专业版本 page app 开发
*** issue: applications over the unaudited use restrictions
- http://open.weibo.com/qa/index.php?qa=159&qa_1=%E7%94%A8%E6%88%B7%E6%8E%88%E6%9D%83%E5%90%8E%E5%87%BA%E7%8E%B0applications-over-unaudited-restrictions%E9%94%99%E8%AF%AF%E6%80%8E%E4%B9%88%E8%A7%A3%E5%86%B3%EF%BC%9F

*** issue: 自适应高度, http://open.weibo.com/wiki/%E4%B8%93%E4%B8%9A%E7%89%88%E5%BA%94%E7%94%A8%E5%BC%80%E5%8F%91%E6%8C%87%E5%8D%97
** cookie
...
*** 代码实例:
# 写cookie
  def do_login(user, tp_account)
    cookies[:login] = {
      value: {uid: tp_account.uid, user_id: user.id}.to_json,
      expires: tp_account.expires_at
    }
  end

# 读cookie

  def login_cookie(key)
    if cookies[:login]
      @login = JSON.parse cookies[:login]
      @login[key]
    end
  end
** request
- request.url
http://weibopage.fchk.dev/appPage?cid=2791328227&ifmID=ifm&url=http%3A%2F%2Fe.weibo.com%2Fproxy.html&key=&sub_appkey=3007443837&viewer=2696072061&tokenString=QAaa68UHDDSlXZUvGLmGE5CC1Da-qNLfmVGXAnn4zF0.eyJ1c2VyIjp7ImNvdW50cnkiOiJjbiIsImxvY2FsZSI6IiJ9LCJhbGdvcml0aG0iOiJITUFDLVNIQTI1NiIsImlzc3VlZF9hdCI6MTM5NTAyNDY0NH0
- request.GET
{"cid"=>"2791328227", "ifmID"=>"ifm", "url"=>"http://e.weibo.com/proxy.html", "key"=>"", "sub_appkey"=>"3007443837", "viewer"=>"2696072061", "tokenString"=>"QAaa68UHDDSlXZUvGLmGE5CC1Da-qNLfmVGXAnn4zF0.eyJ1c2VyIjp7ImNvdW50cnkiOiJjbiIsImxvY2FsZSI6IiJ9LCJhbGdvcml0aG0iOiJITUFDLVNIQTI1NiIsImlzc3VlZF9hdCI6MTM5NTAyNDY0NH0"}
- request.query_string
cid=2791328227&ifmID=ifm&url=http%3A%2F%2Fe.weibo.com%2Fproxy.html&key=&sub_appkey=3007443837&viewer=2696072061&tokenString=QAaa68UHDDSlXZUvGLmGE5CC1Da-qNLfmVGXAnn4zF0.eyJ1c2VyIjp7ImNvdW50cnkiOiJjbiIsImxvY2FsZSI6IiJ9LCJhbGdvcml0aG0iOiJITUFDLVNIQTI1NiIsImlzc3VlZF9hdCI6MTM5NTAyNDY0NH0
...
*** 实例
contest.lotteries.where(user_id: win_lottery.user_id).update_all(status: LotterySt['未中奖'].code)

** rbenv支持whenever
- http://stackoverflow.com/questions/8434922/ruby-script-using-rbenv-in-cron
whenever 是通过 bash shell来执行定时脚本的， /bin/bash -lc
所以我们在~/.bashrc中加入代码
export PATH="$HOME/.rbenv/bin:$PATH"
eval "$(rbenv init -)"
即可让bash识别rbenv，从而使rbenv支持whenever

** Preload、 Eagerload、 Includes 和 Joins
- http://ruby-china.org/topics/17866

** distinct
- http://ruby-china.org/topics/17866
User.joins(:posts).select('distinct users.*').to_a
*** 代码实例
contest.votes.select('distinct votes.user_id').count

** config.assets.precompile
*** 实例
lib/assets/images目录下有alert子目录，alert子目录有各种图片文件
config.assets.precompile += ['swfupload/swfupload.swf', 'alert/*.png']
** assets precompile
...
** order 排序
*** 组合排序
current_user.contests.order('homepage_flag desc, end_day desc')
